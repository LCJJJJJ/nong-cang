...
class OutboundRecordControllerTests {
	...
	@Test
	void shouldReturnOutboundRecordList() throws Exception {
		mockMvc.perform(get("/api/outbound-record/list")
					.header(HttpHeaders.AUTHORIZATION, bearerToken())
					.param("recordCode", "OR-202604190001"))
				.andExpect(status().isOk())
				.andExpect(jsonPath("$.success").value(true))
				.andExpect(jsonPath("$.data", Matchers.hasSize(1)))
				.andExpect(jsonPath("$.data[0].recordCode").value("OR-202604190001"));
	}

	@Test
	void shouldGenerateOutboundRecordAfterComplete() throws Exception {
					...
					.content("""
							{
							  "customerId": 1,
							  "warehouseId": 2,
							  "expectedDeliveryAt": "2026-04-24T09:00:00",
							  "remarks": "记录测试",
							  "items": [
							    {
							      "productId": 3,
		...
		mockMvc.perform(get("/api/outbound-record/list")
				...
				.andExpect(status().isOk())
				.andExpect(jsonPath("$.data[0].outboundOrderId").isNotEmpty());
	}
